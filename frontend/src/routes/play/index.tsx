...
export default component$(() => {
    ...
    const buildingCosts = {
        ...
        powerPlant: { wood: 10000, minerals: 2000, rareMinerals: 25, energyCrystals: 5 }
    };

    const calculateRates = $(() => {
        const { wheatFarms, houses, woodcutters, mines, sawmills, castles, crystalMine, powerPlant } = gameState.value.buildings;
        ...
        const woodProduction = woodcutters * (5 / cyclesPerHour) * (1 + (sawmills * 0.5));
        ...
        const rareMineralsConsumption = (crystalMine + powerPlant) * (2 / cyclesPerHour);
    ...
    const canAfford = $((
        cost: { coins?: number; wood?: number; minerals?: number; wheat?: number; rareMinerals?: number; energyCrystals?: number }
    ): boolean => {
        ...
        return (!cost.coins || userCoins >= cost.coins) &&
    ...
    const purchaseBuilding = $(async (building: keyof GameState['buildings']) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                await updateGameState({[building]: gameState.value.buildings[building] + 1});
                if (user.resources) {
                ...
                console.log("Building constructed successfully!");
            } else {
    ...
    useTask$(async () => {
        try {
            if (user.email) {
                const response = await fetch(`http://localhost:5000/api/game/state?userId=${user.email.toLowerCase()}`);
                if (response.ok) {
                    const data = await response.json();
                    if (data.gameState) {
                        gameState.value = {
                            ...gameState.value,
                            buildings: { ...gameState.value.buildings, ...data.gameState.buildings }
                        };
    ...
    const syncResources = $(async () => {
        ...
        Object.entries(resourceMap).forEach(([oldKey, newKey]) => {
            if (user.resources && user.resources[oldKey as keyof typeof user.resources] !== undefined) {
    ...
    return (
        <div style={{ padding: "2rem", maxWidth: "1200px", margin: "0 auto" }}>
            <h1 style={{ textAlign: "center", fontSize: "3rem", marginBottom: "2rem" }}> Kingdom Builder</h1>
            ...
            <div style={{
                ...
                <div style={{ background: "#fff9c4", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                    <div> Wheat</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{formatNumber(user.resources?.wheat || 0)}</div>
                    <small>➚ {formatNumber(gameState.value.productionRates.wheat)}/h</small>
                    <small style={{ display: "block", color: "#d32f2f" }}>➘ {formatNumber(gameState.value.consumptionRates.wheat)}/h</small>
                </div>

                <div style={{ background: "#e8f5e8", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                    <div> Population</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{formatNumber(gameState.value.productionRates.population)}</div>
                    <small>Capacity</small>
                </div>

                <div style={{ background: "#d7ccc8", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                    <div> Wood</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{formatNumber(user.resources?.wood || 0)}</div>
                    <small>➚ {formatNumber(gameState.value.productionRates.wood)}/h</small>
                </div>

                <div style={{ background: "#bbdefb", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                    <div>⛏️ Minerals</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{formatNumber(user.resources?.mineral || 0)}</div>
                    <small>➚ {formatNumber(gameState.value.productionRates.minerals)}/h</small>
                    <small style={{ display: "block", color: "#d32f2f" }}>➘ {formatNumber(gameState.value.consumptionRates.minerals)}/h</small>
                </div>

                <div style={{ background: "#ffd700", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                    <div> Coins</div>
                    <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{formatNumber(user.coins || 0)}</div>
                </div>
            </div>
            ...
            {(user.resources?.mineralRare || user.resources?.energyCrystals) && (
                <div style={{
                    ...
                    {user.resources?.mineralRare && (
                        <div style={{ background: "#f3e5f5", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                            <div> Rare Minerals</div>
                            <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{user.resources.mineralRare}</div>
                            <small>➚ {formatNumber(gameState.value.productionRates.rareMinerals)}/h</small>
                            <small style={{ display: "block", color: "#d32f2f" }}>➘ {formatNumber(gameState.value.consumptionRates.rareMinerals)}/h</small>
                        </div>
                    )}

                    {user.resources?.energyCrystals && (
                        <div style={{ background: "#e1f5fe", padding: "1rem", borderRadius: "0.5rem", textAlign: "center" }}>
                            <div>⚡ Energy Crystals</div>
                            <div style={{ fontSize: "1.5rem", fontWeight: "bold" }}>{user.resources.energyCrystals}</div>
                            <small>➚ {formatNumber(gameState.value.productionRates.energyCrystals)}/h</small>
                            <small style={{ display: "block", color: "#d32f2f" }}>➘ {formatNumber(gameState.value.consumptionRates.energyCrystals)}/h</small>
                        </div>
                    )}
                </div>
            )}
            ...
            <div style={{ marginTop: "2rem", display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))" }}>
                <div style={{ padding: "1rem", background: gameState.value.productionRates.wheat > gameState.value.consumptionRates.wheat ? "#e8f5e8" : "#ffcdd2", borderRadius: "0.5rem" }}>
                    <strong>Food Balance:</strong> {gameState.value.productionRates.wheat > gameState.value.consumptionRates.wheat ? "✅ Surplus" : "❌ Deficit"}
                </div>

                <div style={{ padding: "1rem", background: gameState.value.buildings.castles > 0 ? "#e8f5e8" : "#fff9c4", borderRadius: "0.5rem" }}>
                    <strong>Kingdom Level:</strong> {gameState.value.buildings.castles > 0 ? "🏰 Advanced" : "🏠 Basic"}
                </div>

                {gameState.value.buildings.powerPlant > 0 && (
                    <div style={{ padding: "1rem", background: "#e1f5fe", borderRadius: "0.5rem" }}>
                        <strong>Power Status:</strong> ⚡ Powered ({gameState.value.buildings.powerPlant} plants)
                    </div>
                )}
            </div>

            {/* Game Tips */}
            <div style={{ marginTop: "2rem", padding: "1rem", background: "#e0f7fa", borderRadius: "0.5rem" }}>
                <h3> Game Tips</h3>
                <ul>
                    <li> Wheat feeds your population - keep production higher than consumption!</li>
...
const BuildingCard = component$((props: BuildingCardProps) => {
    ...
    return (
        <div style={{
            ...
            alignItems: "center",
            background: props.cost.rareMinerals || props.cost.energyCrystals ? "#f3e5f5" : "white"
        }}>
            <div style={{ flex: 1 }}>
                ...
                <p style={{ margin: "0", fontSize: "0.9rem" }}><strong>Built:</strong> {props.current}</p>
            </div>
            <button
                onClick$={props.onPurchase$}
                style={{
                    padding: "0.5rem 1rem",
                    background: (props.cost.rareMinerals || props.cost.energyCrystals) ? "#9c27b0" : "#4CAF50",
                    color: "white",
                    border: "none",
            ...
            >
                {(props.cost.rareMinerals || props.cost.energyCrystals) ? "⚡ Build" : "Build"}
            </button>
        </div>
    );
});