...
export const Item = component$((props: any) => {
    ...
    const handleCheckout = $(async () => {
        try {
            ...
            const resourceType = resourceMap[props.resourceType] || props.resourceType.toLowerCase();

            console.log(props);
            const response = await fetch("http://localhost:5000/api/payment/coinsPayment", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    return (
        <div style={{
            ...
            <form preventdefault:submit onSubmit$={handleCheckout}>
                <button disabled={(coins ? (coins < props.price) : false)} type="submit" style={{cursor: "pointer", width: "100%"}}>
                    Buy
                </button>
            </form>
        </div>
