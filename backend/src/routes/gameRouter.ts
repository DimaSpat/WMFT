    ...
    try {
        ...
        if (!userData) {
            ...
            const newUser = {
                coins: 1000,
                resources: {
                    wheat: 100,  // Changed to lowercase
                    wood: 200,   // Changed to lowercase
                    mineral: 50, // Changed to lowercase
                    mineralRare: 0,
                    energyCrystals: 0
                },
                gameState: {
...
gameRouter.post("/build", async (c) => {
    try {
        ...
        if (cost.wood && (resources.wood || 0) < cost.wood) {  // Changed to lowercase
            return c.json({ error: 'Insufficient wood' }, 400);
        }
        if (cost.minerals && (resources.mineral || 0) < cost.minerals) { // Changed to lowercase
            return c.json({ error: 'Insufficient minerals' }, 400);
        }
        if (cost.wheat && (resources.wheat || 0) < cost.wheat) { // Changed to lowercase
        ...
        if (cost.energyCrystals && (resources.energyCrystals || 0) < cost.energyCrystals) {
        ...
        user.gameState.buildings[buildingType] = (user.gameState.buildings[buildingType] || 0) + 1;
